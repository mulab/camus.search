...
conn.put_mapping(doc_type=doc_type, mapping=mapping, indices=[index])
        ...
        for filename in filenames:
            ...
            if objInfo is not None:
                ...
                if not conn.exists(index=index, doc_type=doc_type, id=dirID + '_' + pid):
                    ...
                    conn.update(whole, index=index, doc_type=doc_type, id=dirID + '_' + pid)

conn.optimize(indices=['qa'])
conn.refresh()
